...
filled in
    numbers while others have not. Cells that have not been filled in hold the 
potential
    values that could be assigned to the cell (i.e. have not been ruled out from 
the
    row, column or subgrid)
    Attributes:
        num_nums_placed - number of numbers placed so far (initially 0)
        size - the size of the board (this will always be 9, but is convenient to 
have
            an attribute for this for debugging purposes)
        rows - a list of 9 lists, each with 9 elements (imagine a 9x9 sudoku 
board).
            Each element will itself be a list of the numbers that remain possible 
to
            assign in that square. Initially, each element will contain a list of 
the
            numbers 1 through 9 (so a triply nested 9x9x9 list to start) as all 
numbers
            are possible when no assignments have been made. When an assignment is 
made
...
ties
        ...
        col=0
        row = 0
        shortest_len = self.size
        for r in range(self.size):
            for c in range(self.size):
                if isinstance(self.rows[r][c], list):
                    if len(self.rows[r][c]) < shortest_len:
                        shortest_len = len(self.rows[r][c])
                        col = c
                        row = r
        return(row, col)
...
assigning
        ...
        """
        #print("Updating", row, column, "=", assignment)
        #print(self.rows[row][column])
        self.rows[row][column]= assignment
        self.num_nums_placed += 1
        #doing a fewthings
        #setting an assignment
        #romoving this assignment from other possibility lists in the:
        # - row
        for cell_contents in self.rows[row]:
            remove_if_exists(cell_contents, assignment)
        # - col
        for i in range(0,9):
            cell_contents = self.rows[i][column]
            remove_if_exists(cell_contents, assignment)
        # - subgrid
        all_subgrid_coords = self.subgrid_coordinates(row, column)
        for (coord_row, coord_col) in all_subgrid_coords:
            cell = self.rows[coord_row][coord_col]
            remove_if_exists(cell, assignment)
        #for loop that counts from 0 to 9 to count through rows and columns
        #subgrid is list of cells
        #for i in range(0, 9):
        #print(self.rows[row])
        pass
...
constrained
    ...
    stack = Stack()
    stack.push(state)
    #print("adding to the state")
    #while the stack isnt empty
    while stack.is_empty() == False:
        curr = stack.pop()
        if curr.goal_test() == True:
            #print("we found a solution")
            return curr
        elif curr.failure_test() == True:
            #print("we reached a dead end")
            continue
        elif curr.failure_test() == False:
            #print("testing")
            constrained_cell = curr.find_most_constrained_cell()
            for i in curr.rows[constrained_cell[0]][constrained_cell[1]]:
                cpy = copy.deepcopy(curr)
                cpy.update(constrained_cell[0], constrained_cell[1], i)
                stack.push(cpy)
    return None
