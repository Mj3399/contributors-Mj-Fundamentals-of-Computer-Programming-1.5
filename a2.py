    ...
    while sind != len(source) or pind != len(pattern):
        ...
        if pind >= len(pattern):
            if accumulating == True:
                accumulator.append(source[sind])
                sind += 1
            else:
                return None
...
conditions
        #   and come back to this one afterwards
            ...
            if pind >= len(pattern)-1:
                #print("nothing after accumulator")
                accumulator += " ".join(source[sind:])
                #print("accumulator", accumulator)
                return result + [accumulator]
            else:
                stop_symbol = pattern [pind +1]
                #print("something after accumulator", stop_symbol)
                while source[sind] != stop_symbol:
...
in the
        # source
            ...
            if accumulating == True:
               accumulator = accumulator + str(source[sind])
    ...
    #assert match(["x", "%", "z"], ["x", "y", "z", "z", "z"]) == None, "test 15 
failed"
